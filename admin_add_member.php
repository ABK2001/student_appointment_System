<?php
$page_title = "Add new faculty member";
?>
...
if (isset ( $_POST ['btn-submit'] )) {
	...
	$check_query = "SELECT * FROM faculty_member WHERE email = '$email' OR username = '$username' OR mobile = '$mobile'";
	$check_result = mysqli_query($con, $check_query);
	
	if (mysqli_num_rows($check_result) > 0) {
		echo "<script>alert('Error: Email, username or mobile already exists');</script>";
	} else {
		if (mysqli_query ($con,  "INSERT INTO faculty_member (first_name, last_name, username, email, password, mobile, gender, age, speciality, department) VALUES ('$first_name', '$last_name', '$username', '$email', '$password', '$mobile', '$gender', '$age', '$speciality', '$department')" )) {
...
<div class="contact" data-aos="fade-up">
	<form method="post" role="form" class="php-email-form" enctype="multipart/form-data">
		...
		<div class="form-group mt-3">
			Age
			<input type="number" class="form-control" name="age" placeholder="Age" min="25" max="70" required />
		</div>
		<div class="form-group mt-3">
			Speciality
